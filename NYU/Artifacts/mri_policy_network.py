...
parser.add_argument('--env_name', type=str, default = 'MRI')
...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(n_layers):
            h = tf.layers.dense(h, size, activation=activation, kernel_initializer = tf.initializers.random_normal, name='h{}'.format(i + 1))
...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        #print(tf.shape(sy_sampled_ac))
        #print(self.sessy_sampled_ac)
    ...
    def sample_trajectories(self, itr, env):
        ...
        while True:
            ...
            #print("Starting episode " + str(len(paths)))
            path = self.sample_trajectory(env, animate_this_episode)
            paths.append(path)
            timesteps_this_batch += pathlength(path)
            if timesteps_this_batch > self.min_timesteps_per_batch or len(paths) >= 10:
                print(paths[len(paths) - 1]["observation"])
                break
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            #====================================================================================#
            # YOUR CODE HERE
            ac, pp = self.sess.run([self.sy_sampled_ac, self.policy_parameters], feed_dict={self.sy_ob_no: ob.reshape(1, -1)})
            #ac1 = np.argmin(pp[0])
            #ac2 = np.argmin(pp[1])
            #print(pp)
            ac = ac[0]
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        for i in range(self.pg_step):
            ...
            ac, pp = self.sess.run([self.sy_sampled_ac, self.policy_parameters], feed_dict={
                self.sy_ob_no:ob_no,
                self.sy_ac_na:ac_na,
                self.sy_adv_n:adv_n
            })
